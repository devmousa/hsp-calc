...
const Literary = () => {
  ...
  const onSubmit: SubmitHandler<ISubjects> = (event) => {
    ...
    Swal.fire({
      title: "%" + percentage,
      html: `نسبتك هي ${percentage}% <br />التقدير: <b>${appreciation}</b>`,
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="p-2 flex flex-wrap md:flex-row flex-col md:gap-4 gap-y-4 justify-center md:items-center">
          <SubjectInput register={register("TwoSemesters")} subjectEnglishName="TwoSemesters" subject="مجموع الفترتين" max={372} sumOfSemesters={true} />
          <SubjectInput register={register("IslamicEducation")} subjectEnglishName="IslamicEducation" subject="التربية الإسلامية" max={48} />
          <SubjectInput register={register("LinguisticStudies")} subjectEnglishName="LinguisticStudies" subject="الدراسات اللغوية" max={60} />
          <SubjectInput register={register("LiteraryStudies")} subjectEnglishName="LiteraryStudies" subject="الدراسات الأدبية" max={60} />
          <SubjectInput register={register("ReadingAndLiteraryCriticism")} subjectEnglishName="ReadingAndLiteraryCriticism" subject="المطالعة و النقد الأدبي" max={60} />
          <SubjectInput register={register("Expression")} subjectEnglishName="Expression" subject="الإنشاء" max={20} expressionsSubject={true} />
          ...
          <SubjectInput register={register("History")} subjectEnglishName="History" subject="التاريخ" max={60} />
          <SubjectInput register={register("Geography")} subjectEnglishName="Geography" subject="الجغرافيا" max={60} />
          <SubjectInput register={register("Psycology")} subjectEnglishName="Psycology" subject="علم النفس" max={60} />
          <SubjectInput register={register("Sociology")} subjectEnglishName="Sociology" subject="علم الاجتماع" max={60} />
          <SubjectInput register={register("Philosophy")} subjectEnglishName="Philosophy" subject="الفلسفة" max={60} />
        </div>
